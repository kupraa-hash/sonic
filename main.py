...
def request_otp(email, delay=30):
    try:
        response = requests.post('https://api.pixelverse.xyz/api/otp/request', json={'email': email})
        print(f"Request OTP response for {email}: {response.status_code}, {response.text}")
        if response.status_code == 200:
            return True
        elif response.status_code == 429:
            print("Throttling detected. Waiting before retrying...")
            time.sleep(delay)
